                ...
                <div class="col-lg-8 table-responsive mb-5">
                    <table class="table table-bordered text-center mb-0">
                        ...
                        <tbody class="align-middle">
                            ...
                            $total = 0;
                            $gtotal = 0;
                            $stmt = $admin->ret("SELECT * FROM `cart` INNER JOIN `usedbook` ON usedbook.ubook_id=cart.ubook_id WHERE cart.user_id='$uid'");
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                $cqty = $row['cart_qty'];
                                $bprice = $row['ubook_price'];
                                $total = $bprice * $cqty;
                                $gtotal = $gtotal + $total;
                            ?>
                                <tr>
                                    <td class="align-middle"><img src="controller/<?php echo $row['ubook_img'] ?>" alt="" style="width: 160px;"> </td>

                                    <td class="align-middle">₹<?php echo $row['ubook_price'] ?></td>
                                    <td>
                                        <div class="col" style="display: flex;margin-top:100px">
                                            ...
                                            <button class="btn btn-sm btn-primary" onclick="increment(<?php echo $row['ubook_qty'] ?>,<?php echo $row['cart_id'] ?>)">+</button>
                                        </div>
                                    </td>
                                    <td class="align-middle">₹<?php echo $total ?></td>
                                    <td class="align-middle"><a href="controller/deleteusedcart.php?cid=<?php echo $row['cart_id'] ?>" class="btn btn-sm btn-primary"><i class="fa fa-times"></i></a></td>
                                </tr>
                            <?php }
                            ?>
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card border-secondary mb-5">
                        ...
                        <div class="card-body">
                            ...
                            $total2 = 0;
                            $gtotal2 = 0;
                            $stmt2 = $admin->ret("SELECT * FROM `cart` INNER JOIN `usedbook` ON usedbook.ubook_id=cart.ubook_id WHERE cart.user_id='$uid'");
                            while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
                                $cqty2 = $row2['cart_qty'];
                                $bprice2 = $row2['ubook_price'];
                                $total2 = $bprice2 * $cqty2;
                                $gtotal2 = $gtotal2 + $total2;
                            ?>
                                <div class="d-flex justify-content-between mb-3 pt-1">
                                    <h6 class="font-weight-medium"><?php echo $row2['ubook_title'] ?></h6>
                        ...
                        <div class="card-footer border-secondary bg-transparent">
                            ...
                            <a href="usedbookcheckout.php" class="btn btn-block btn-primary my-3 py-3">Proceed To Checkout</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        function increment(stock, cartid) {
            ...
            if (qty > stock) {
                ...
                xmlhttp.open("GET", "controller/updateusedcart.php?cartid=" + cartid + '&qty=' + qty, true);
                xmlhttp.send();
