...
if (isset($_POST['checkout'])) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $stmt2 = $admin->ret("SELECT * FROM `cart` INNER JOIN `book` ON book.book_id=cart.book_id INNER JOIN `user` ON user.user_id=cart.user_id WHERE user.user_id='$uid'");
            while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
               ...
               $bookqty = $row2['book_qty'];
                $remainingqty = $bookqty - $cqty;

                $stmt8 = $admin->cud("UPDATE `book` SET `book_qty`='$remainingqty' WHERE `book_id`='$bookid'", "updated");

                $stmtOrder = $admin->Rcud("INSERT INTO `order`(`user_id`,`seller_id`,`book_id`,`qty`,`amt`,`od_status`,`unid`,`od_date`)VALUES('$uid','$sellerid','$bookid','$cqty','$total','ordered','$un',now())");
                ...
                $stmt5 = $admin->cud("DELETE FROM `cart` WHERE `user_id`='$uid'","deleted");
                // echo "<script>window.location='../thankyou.php';</script>";
            }
       
    }
    $stmt6=$admin->ret("SELECT * FROM `user` WHERE `user_email`='$email' ");
$num=$stmt6->rowCount();
if($num>0){
    echo "email  exist";
   
    echo $_SESSION['supemail']=$email;
    header("location:../phpmailer/index2.php?email=$email");
}
}
...
if (isset($_POST['usedbookcheckout'])) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $stmt2 = $admin->ret("SELECT * FROM `cart` INNER JOIN `usedbook` ON usedbook.ubook_id=cart.ubook_id INNER JOIN `user` ON user.user_id=cart.user_id WHERE user.user_id='$uid'");
            while ($row2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
